...
namespace AutoRest.ObjC
{
    public static class Extensions
    {
        ...
        public static string ToShortName(this string longName)
        {
            ...
            return CodeNamerObjC.Instance.GetVariableName(acronym);
        }
        ...
        {
            var r = body as CompositeTypeObjC;
        ...
        {
            if (type is PrimaryTypeObjC)
            {
                return (type as PrimaryTypeObjC).GetEmptyCheck(valueReference, asEmpty);
            }
            else if (type is ArrayTypeObjC)
            {
                return (type as ArrayTypeObjC).GetEmptyCheck(valueReference, asEmpty);
            }
            else if (type is DictionaryTypeObjC)
            {
                return (type as DictionaryTypeObjC).GetEmptyCheck(valueReference, asEmpty);
            }
            else if (type is EnumTypeObjC)
            {
                return (type as EnumTypeObjC).GetEmptyCheck(valueReference, asEmpty);
            }
            else
            {
        ...
        {
            if (type is DictionaryTypeObjC)
            {
                (type as DictionaryTypeObjC).AddImports(imports);
            }
            else if (type is PrimaryTypeObjC)
            {
                (type as PrimaryTypeObjC).AddImports(imports);
            }
            else if (type is ArrayTypeObjC)
            {
                (type as ArrayTypeObjC).AddImports(imports);
            }
        }
        ...
        {            
            return (type is PrimaryType || type is SequenceType || type is DictionaryType || type is EnumType || 
                (type is CompositeTypeObjC && (type as CompositeTypeObjC).IsPolymorphicResponse()));
        }
            ...
            bool isCompositeProperties = false)
        {
            ...
            if (method != HttpMethod.Patch || !p.IsBodyParameter() || isCompositeProperties)
            {
                foreach (var prop in ((CompositeType)p.ModelType).Properties)
                {
                    ...
                    var map = prop.ModelType as DictionaryTypeObjC;
